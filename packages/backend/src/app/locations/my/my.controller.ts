...
export class MyController {
  constructor(private readonly myService: MyService, private readonly locationsService: LocationsService) {}

  @Get()
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Get my by user id' })
  @ApiResponse({ status: 201, description: 'List of my locations.', type: GetMyLocationsResponseDTO })
  ...
  async getMy(@Query('offset') offset, @Request() req: FRequest) {
    ...
    const mappedFavorites = await Promise.all(my.map(async (location) => {
      const loc = location;
