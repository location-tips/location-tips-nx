...
export class LocationsSetService {
  async saveLocationsSetToDB(
    ...
    const ref = await db
      .collection(COLLECTIONS.LOCATIONS_SETS)
      .add(locationsSet);
  ...
  async updateLocationsSetInDB({
    id,
    ...restData
  }: PutLocationsSetRequest): Promise<TLocationsSet> {
    const db = admin.firestore();
  ...
  private async updateEmbeddingsInLocationsSet(
    ...
    const locationsDocs = await db
      ...
      .where(firestore.FieldPath.documentId(), 'in', locationIds.slice(0, 30))
      .get();
