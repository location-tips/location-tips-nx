import { Body, Controller, Delete, Post, Get, Query, UseGuards, Request } from '@nestjs/common';
...
export class FavouritesController {
  ...
  @ApiResponse({ status: 201, description: 'List of favourites locations.', type: GetFavouritesResponseDTO })
  ...
  @ApiBearerAuth()
  @ApiResponse({ status: 201, description: 'The record has been successfully updated.', type: PostFavouritesResponseDTO })
  @ApiResponse({ status: 400, description: 'Empty request.'})
  ...
  async PostFavourites(@Body() data: PostFavouritesRequestDTO, @Request() req: FRequest) {
    const doc = await this.favouritesService.getUserFavourites(req.user.uid);
  ...
  @ApiBearerAuth()
  @ApiResponse({ status: 201, description: 'The record has been successfully deleted.', type: DeleteFavouritesResponseDTO })
  @ApiResponse({ status: 400, description: 'Empty request.'})
  @ApiResponse({ status: 403, description: 'Forbidden.'})
  @ApiResponse({ status: 500, description: 'Server error.'})
  @UseGuards(AuthGuard)
  ...
  async deleteFavourites(@Body() { locationId }: DeleteFavouritesRequestDTO, @Request() req: FRequest) {
    const doc = await this.favouritesService.getUserFavourites(req.user.uid);
