...
export const geminiDescribeSearchQuery = async (
  ...
  const model = genAI.getGenerativeModel({
    model: 'gemini-1.5-pro',
    generationConfig: { responseMimeType: 'application/json' },
    systemInstruction: `Very important: Answer should be a VALID JSON. User provides you prompt to find some location, also it could provide and image or audio you need to understand it and output in the valid json format with following fields:
    {
